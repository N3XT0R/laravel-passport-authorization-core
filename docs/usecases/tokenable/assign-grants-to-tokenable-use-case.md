...
Assigns a list of scope grants to a tokenable (owner) in the context of a specific client and dispatches a
`TokenableGrantsAssignedEvent`.
...
- `N3XT0R\LaravelPassportAuthorizationCore\Events\Tokenable\TokenableGrantsAssignedEvent` is dispatched after the grants
  are assigned.
