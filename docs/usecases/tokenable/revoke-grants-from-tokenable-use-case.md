...
Revokes a list of scope grants from a tokenable (owner) in the context of a specific client and dispatches a
`TokenableGrantsRevokedEvent`.
...
- `N3XT0R\LaravelPassportAuthorizationCore\Events\Tokenable\TokenableGrantsRevokedEvent` is dispatched after the grants
  are revoked.
