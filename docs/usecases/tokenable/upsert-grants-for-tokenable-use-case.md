...
Upserts a list of scope grants for a tokenable (owner) in the context of a specific client and dispatches a
`TokenableGrantUpsertedEvent`.

## When to use

Use this when you want the tokenable's grants for a given client context to match the provided list (adding missing
grants and removing unlisted ones).
...
- `N3XT0R\LaravelPassportAuthorizationCore\Events\Tokenable\TokenableGrantUpsertedEvent` is dispatched after the grants
  are upserted.
