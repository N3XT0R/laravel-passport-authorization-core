...
## Purpose
Retrieves owners as a key/value collection suitable for relationship dropdowns. The keys come from the owner model's primary key, and the values use the configured label attribute.
...
## Output
- Returns an `Illuminate\Support\Collection<int|string, string>` keyed by the owner model key and valued by the configured label attribute.
