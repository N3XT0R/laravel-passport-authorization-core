...
Creates a new Passport scope action via the `ActionService` and emits an `ActionCreatedEvent` once the action is
persisted.

## When to use

Use this when you want to register a new action that can be attached to scope resources (for example, `read`, `write`,
...
- `N3XT0R\LaravelPassportAuthorizationCore\Events\PassportScopeAction\ActionCreatedEvent` is dispatched if the action
  was persisted.
